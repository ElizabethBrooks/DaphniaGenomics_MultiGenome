...
for i in /temp180/mpfrende/ebrooks5/multi_genome/EGAPx_v0.3.2/*/*/*/AGAT_v1.4.2/lncRNAs.fa; do dirName=$(dirname $i); newDir=$(dirname $dirName); newName=$(basename $newDir); seqMean=$(cat $i | grep ">" | cut -d" " -f2 | sort | uniq -c | wc -l); echo $newName","$seqMean >> $outFile; done
for i in /temp180/mpfrende/ebrooks5/multi_genome/EGAPx_v0.3.2/*/*/AGAT_v1.4.2/lncRNAs.fa; do dirName=$(dirname $i); newDir=$(dirname $dirName); newName=$(basename $newDir); seqMean=$(cat $i | grep ">" | cut -d" " -f2 | sort | uniq -c | wc -l); echo $newName","$seqMean >> $outFile; done
for i in /temp180/mpfrende/ebrooks5/multi_genome/EGAPx_v0.5.2/*/*/AGAT_v1.4.2/lncRNAs.fa; do dirName=$(dirname $i); newDir=$(dirname $dirName); newName=$(basename $newDir); seqMean=$(cat $i | grep ">" | cut -d" " -f2 | sort | uniq -c | wc -l); echo $newName","$seqMean >> $outFile; done
