...
for i in /Users/bamflappy/PfrenderLab/multi_genome_project/evaluation_tree_species/EGAPx_v0.5.2/*/AGAT_v1.4.2/annotation_stats.txt; do echo $i; dirName=$(dirname $i); newDir=$(dirname $dirName); newName=$(basename $newDir); baseName=$(basename $i); cat $i | grep -A35 "\- lncrna \-" | tail -35 | sed "s/^/$newName,/g" | sed "s/(.*)//g" | tr ' ' '_' | tr -s '_' | sed "s/90_/ninety_/g" | sed "s/_[0-9]/,&/" | sed "s/_,/,/g" | tr -s ',' | sed "s/,_/,/g" > /Users/bamflappy/PfrenderLab/multi_genome_project/evaluation_tree_species/EGAPx_v0.5.2/AGAT_v1.4.2/$newName"_lnc_rna_"$baseName; done
